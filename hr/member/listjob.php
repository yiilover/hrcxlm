...
if($job=="list"){	//管理职位
	...
	$min=($page-1)*$rows;
	$query = $db->query("SELECT * FROM {$_pre}content WHERE uid='$lfjuid' ORDER BY list DESC LIMIT $min,$rows");
	...
	$db->get_one("UPDATE `{$_pre}content` SET list='$timestamp',posttime='$timestamp' WHERE id='$id' AND uid=$lfjuid");
