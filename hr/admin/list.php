...
if($job=="list")
{
	...
	$query = $db->query("SELECT SQL_CALC_FOUND_ROWS A.*,B.title AS company FROM {$_pre}content A LEFT JOIN {$pre}hy_company B ON A.uid=B.uid $SQL ORDER BY A.id DESC LIMIT $min,$rows");
	...
	while($rs = $db->fetch_array($query)){
		...
		$rs[ifyz] = $rs[yz]?"<A HREF='$admin_path&action=work&jobs=unyz&id=$rs[id]'>已审</A>":"<A HREF='$admin_path&action=work&jobs=yz&id=$rs[id]' style='color:blue;'>未审</A>";
		$listdb[]=$rs;
	}

	$showpage=getpage("","","$admin_path&job=list&fid=$fid",$rows,$totalNum);
