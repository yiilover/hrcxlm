...
function showmorecontent($mid){
	global $db,$_pre,$rsdb,$pre;
	$show="";
	$query = $db->query("SELECT * FROM `{$_pre}field` WHERE mid='$mid' ORDER BY orderlist DESC,id ASC");
	@extract($db->get_one("SELECT title AS company FROM `{$pre}hy_company` WHERE uid='$rsdb[uid]'"));	
	$show.= "<div class='lists'><span>招聘公司:</span><a href=\"../../home/?uid=$rsdb[uid]\" style=\"color:red;\" target=\"_blank\">".$company."</a></div>\r\n";
