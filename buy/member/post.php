...
if($action=="postnew")
{
	...
	if(!$web_admin){
		...
		if($_rs[NUM]>$groupdb[post_buy_num]){
			showerr("你所在用户组每天发布的求购信息不能超过{$groupdb[post_buy_num]}条,请升级用户组吧");
		}
	}
	...
	//是否自动通过审核
	$postdb[yz] = ($web_admin||$groupdb[buy_postauto_yz])?1:0;

	if($postdb[yz]==1){
		add_user($lfjdb[uid],$webdb[PostInfoMoney],'发布信息奖励');
	}
