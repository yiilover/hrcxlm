...
if($action=="post")
{
	...
	if(!$web_admin&&$webdb['comment_limit']){
		$_rt=$db->get_one("SELECT * FROM {$_pre}join WHERE products LIKE '%$id=%' ORDER BY ifpay DESC LIMIT 1");
		if($webdb['comment_limit']==2){
			if(!$_rt['ifpay']){
				die('必须要付款后,才可以评论!');
			}
		}elseif($webdb['comment_limit']==1){
			if(!$_rt){
				die('必须要下订单后,才可以评论!');
			}
		}
...
elseif($action=="del")
{
	...
	if(!$web_admin&&$rs[uid]!=$lfjuid&&$webdb[DelOtherCommentMoney]){
		...
		add_user($lfjdb[uid],-abs($webdb[DelOtherCommentMoney]),'删除商城信息评论扣分');
	}
