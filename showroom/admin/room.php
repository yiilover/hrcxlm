...
if($job=="list")
{
	...
	$query=$db->query("SELECT SQL_CALC_FOUND_ROWS * FROM {$_pre}room $SQL ORDER BY id DESC LIMIT $min,$rows");
	...
	while($rs=$db->fetch_array($query))
	{
		...
		if(!$rs[levels]){
			...
			$rs[iscom]="<A HREF='$admin_path&action=work&id=$rs[id]&jobs=uncom' style='color:red;'>已推荐</A>";
		}
...
function M_dowork($id,$job){
	...
	$rsdb=$db->get_one("SELECT * FROM {$_pre}room WHERE id='$id' ");
	if($job=="delete")
	{
		//del_info($id,$rsdb);

		$db->query("DELETE FROM  `{$_pre}room` WHERE id='$id'");
		$db->query("DELETE FROM  `{$_pre}content_3` WHERE id='$id'");
	...
	elseif($job=="move")
	{
		...
		if($fid){
			...
			if($rs[mid]==$rsdb[mid]){
				$db->query("UPDATE {$_pre}room SET fid='$fid',fname='$rs[name]',lastfid='$rsdb[fid]' WHERE id='$id' ");
	...
	{
		global $color;
		$db->query("UPDATE {$_pre}room SET titlecolor='$color' WHERE id='$id' ");
	}
	elseif($job=="yz")
	{
		$db->query("UPDATE {$_pre}room SET yz='1' WHERE id='$id' ");
	}
	elseif($job=="unyz")
	{
		$db->query("UPDATE {$_pre}room SET yz='0' WHERE id='$id' ");
	}
	elseif($job=="undel")
	{
		$db->query("UPDATE {$_pre}room SET yz='1' WHERE id='$id' ");
	...
	elseif($job=="com")
	{
		$db->query("UPDATE {$_pre}room SET levels='1',levelstime='$timestamp' WHERE id='$id' ");
	}
	elseif($job=="uncom")
	{
		$db->query("UPDATE {$_pre}room SET levels='0',levelstime='0' WHERE id='$id' ");
	}
	elseif($job=="unyz")
	{
		$db->query("UPDATE {$_pre}room SET levels='0',levelstime='0' WHERE id='$id' ");
	...
	{
		global $toptime;
		$db->query("UPDATE {$_pre}room SET list=list+'$toptime' WHERE id='$id' ");
	}
	elseif($job=="untop")
	{
		$db->query("UPDATE {$_pre}room SET list='$timestamp' WHERE id='$id' ");
	}
}
?>