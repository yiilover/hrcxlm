...
if($action=="search"){
	...
	$SQL="SELECT SQL_CALC_FOUND_ROWS title,id,fid,uid,username,posttime,mid,hits,fname FROM {$_pre}content WHERE $type LIKE '%$keyword%' $sql_fid ORDER BY posttime DESC LIMIT $min,$rows ";
	$query = $db->query("$SQL");
