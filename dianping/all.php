...
$data = array();
$query = $db->query("SELECT A.*,C.title AS companyname FROM {$_pre}content A LEFT JOIN {$pre}hy_company C ON A.uid=C.uid WHERE A.yz='1' ORDER BY $order_by DESC LIMIT $min,$rows");
while($rs = $db->fetch_array($query)){
	...
	$rss[posttime]=date("Y-m-d H:i:s",$rs[posttime]);
	$rss[url]=get_info_url($rs[id],$rs[fid],$rs[city_id]);
	$rss[comments]=$rs[comments];
	$rss[hits]=$rs[hits];		
	$rss[uid]=$rs[uid];
	$data[]=$rss;
}
