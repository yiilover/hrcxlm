...
elseif($action=="list")
{
	...
	elseif($jobs=="yz"||$jobs=="unyz")
	{
		...
		foreach($ciddb AS $key=>$rs){
			$ts = $db->get_one("SELECT * FROM {$_pre}dianping  WHERE cid='$key'");
			if($yz&&!$ts[yz]){
				add_user($ts[uid],$webdb[PostCommentMoney],'发布评论奖励积分');
			}elseif(!$yz&&$ts[yz]){
				add_user($ts[uid],-$webdb[PostCommentMoney],'发布垃圾评论扣积分');
			}
	...
	elseif($jobs=="good"||$jobs=="ungood")
	{
		foreach($ciddb AS $key=>$rs){
			...
			if($jobs=="good"&&$rs[type]!=1){
				...
				add_user($rs[uid],abs($webdb[GoodCommentMoney]),'评论推荐为精华得分');
			}elseif($jobs=="ungood"&&$rs[type]==1){
				$db->query(" UPDATE {$_pre}dianping SET type='0' WHERE cid='$key' ");
				add_user($rs[uid],-abs($webdb[GoodCommentMoney]),'评论取消精华扣分');
			}			
			$ck++;
		}
	}
