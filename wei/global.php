...
function list_content($SQL,$leng=40){
	...
	while( $rs=$db->fetch_array($query) ){
		...
		$rss=$db->get_one("SELECT * FROM {$_pre}content_$rs[mid] WHERE id=$rs[id] LIMIT 1");
		is_array($rss) && $rs=$rs+$rss;

		$rs[content]=@preg_replace('/<([^>]*)>/is',"",$rs[content]);	//把HTML代码过滤掉
		$rs[content]=get_word($rs[full_content]=$rs[content],100);
		if($rs[title]==''){
			$rs[title] = get_txt_word($rs[content],40);
		}
...
*显示发布时间
*/
function List_Post_Time($posttime){
	global $timestamp;
	$showtime=$timestamp-$posttime;
	if($showtime<60){
		$showtimes="在<em>{$showtime}秒前</em>说";
	}elseif($showtime<3600){
		$showtimes=(int)($showtime/60);
		$showtimes="在<em>{$showtimes}分钟前</em>说";
	}elseif($showtime<86400){
		$showtimes=(int)($showtime/3600);
		$mins=(int)(($showtime%3600)/60);
		$mins=$mins?$mins."分":"";
		$showtimes="在<em>{$showtimes}小时{$mins}前</em>说";
	}elseif($showtime<2592000){
		$showtimes=(int)($showtime/86400);
		$showtimes="在<em>{$showtimes}天前</em>说";
	}else{
		$showtimes="在<em>".date("Y-m-d H:i:s",$posttime)."</em>说";
	}
	return $showtimes;
}
...
*分批提取
*/
function List_Dates($listdbs,$min,$max){
	$lists=count($listdbs);
	for($i=$min;$i<$max;$i++){
		if($i>=$lists) break;
		$listdb[]=$listdbs[$i];
