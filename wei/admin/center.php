...
if($job=="config" && ck_power('center_config'))
{
	...
	$allowGroupPost=group_news_box("webdbs[allowGroupPost]",explode(",",$webdb[allowGroupPost]));

	//$GroupPostpassyz=group_news_box("webdbs[GroupPostYZ]",explode(",",$webdb[GroupPostYZ]),1);

	$Info_GroupPostYZ=group_news_box("webdbs[Info_GroupPostYZ]",explode(",",$webdb[Info_GroupPostYZ]));
	...
	$sort_fid=$Guidedb->Select("{$_pre}sort","webdbs[wei_fid]",$webdb[wei_fid]);
