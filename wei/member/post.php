...
if($_FILES||$postdb[picurl]){
	...
	if($ftype[$i]=='in' && $postdb[picurl]&&!eregi("(gif|jpg|png)$",$postdb[picurl])){
		showerr("缩略图,只能上传GIF,JPG,PNG格式的文件,你不能上传此文件:$array[name]");
	}
}
...
if($action=="edit"||$action=="postnew")
{
	...
	$postdb['keywords'] = $postdb['keywords'] ? $postdb['keywords'] : keyword_ck('',$postdb['title']);
}
...
elseif($action=="edit")
{
	...
	if($rsdb['keywords']!=$postdb['keywords']){
		keyword_del($id,$rsdb['keywords']);
		keyword_add($id,$postdb['keywords'],$lfjuid);
	}
