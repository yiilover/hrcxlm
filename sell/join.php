...
if($action=="postnew")
{
	...
	if(!$web_admin){
		if($groupdb[post_xunjiadian_num]<1){
			showerr('你所在用户组不允许发询价单,请升级用户组吧');
		}
		$time=$timestamp-24*3600;
		$_rs=$db->get_one("SELECT COUNT(*) AS NUM FROM `{$_pre}join` WHERE uid='$lfjuid' AND posttime>$time");
		if($_rs[NUM]>$groupdb[post_xunjiadian_num]){
			showerr('你所在用户组每天发布的询价单不能超过{$groupdb[post_xunjiadian_num]}条,请升级用户组吧');
		}
	}
