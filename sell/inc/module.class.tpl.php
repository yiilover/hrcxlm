...
function post_tpl($rs,&$tplcode){
	...
	elseif($rs[form_type]=='ieeditsimp')
	{
		...
		$show0="{$rs[title]}:$mustfill<br>{$rs[form_title]}";
		$show="	<script type='text/javascript' charset='utf-8' src='\$webdb[www_url]/ewebeditor/kindeditor-4.1.9/kindeditor-min.js'></script>
...
<script>
        KindEditor.ready(function(K) {
                window.editor = K.create('#atc_{$rs[field_name]}');
        });
</script>


<textarea name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}' style='width:{$rs[field_inputwidth]}px;height:{$rs[field_inputheight]}px;visibility:hidden;'>\$rsdb[{$rs[field_name]}]</textarea>$rs[form_js]";
	}
...
function show_tpl($rs,&$tplcode){
	//身份证号不显示
	if($rs[field_name]=='idcard') return;
	if($tplcode&&strstr($tplcode,"\$rsdb[{$rs[field_name]}]")){
		return ;
	}
