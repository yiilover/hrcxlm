...
if(!$lfjid){
	showerr('请先登录!');
}elseif($webdb['postShopTgNeedQy']&&!$web_admin&&$lfjdb['grouptype']!=1){
	showerr('只有通过审核的企业用户才能发布团购!');
}
...
{
	/*验证码处理*/
	if(!$web_admin&&$groupdb[postShopTgYzImg]){
	...
	VALUES (
	'$postdb[title]','$fidDB[mid]','$fid','$fidDB[name]','$timestamp','$postdb[list]','$lfjdb[uid]','$lfjdb[username]','$postdb[picurl]','$postdb[ispic]','$postdb[yz]','$postdb[keywords]','$onlineip','$postdb[money]','$postdb[city_id]','$num','$postdb[price]','$postdb[gg_maps]')");
...
elseif($action=="del")
{
	...
	if($rsdb[yz]){
		add_user($lfjdb[uid],-$webdb[PostInfoMoney],'删除官方团信息扣分');
	}
...
elseif($action=="edit")
{
	...
	if($rsdb[yz]==2 && strtotime("$postdb[end_time] 23:59:59")>$timestamp){
		$rsdb[yz] = 1;
	}
	...
	/*更新主信息表内容*/
	$db->query("UPDATE `{$_pre}content` SET title='$postdb[title]',keywords='$postdb[keywords]',picurl='$postdb[picurl]',ispic='$postdb[ispic]',picnum='$num',price='$postdb[price]',gg_maps='$postdb[gg_maps]',yz='$rsdb[yz]',city_id='postdb[city_id]' WHERE id='$id'");
