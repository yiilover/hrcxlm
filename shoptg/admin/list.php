...
if($job=="list")
{
	...
	while($rs=$db->fetch_array($query))
	{
		...
		if(!$rs[yz]){
			...
			$rs[ischeck]="<A HREF='$admin_path&action=work&id=$rs[id]&jobs=undel' style='color:blue;'>已下架</A>";
		}
		if(!$rs[levels]){
...
function dowork($id,$job){
	...
	elseif($job=="punish")
	{
		global $Type;
		if($Type==1){
			add_user($rsdb[uid],-abs($webdb[ErrSortMoney]),'处理官方团信息扣分');
		}elseif($Type==2){
			add_user($rsdb[uid],-abs($webdb[illInfoMoney]),'处理官方团信息扣分');
		}
	}
}
?>