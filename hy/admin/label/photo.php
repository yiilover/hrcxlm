...
if($action=='mod'){
	...
	if($uid_str){
		$SQL.=" AND A.uid IN($uid_str) ";
	}
	
	if($postdb[allcity]){
		$SQL=" LEFT JOIN {$_pre}company B ON A.uid=B.uid $SQL AND B.city_id='\$GLOBALS[city_id]' ";
	}

	$SQL="SELECT A.*,A.url AS picurl  FROM {$_pre}pic A $SQL ORDER BY A.$order $asc LIMIT $sqlmin,$rows ";
