...
$linkdb=array(
			...
			"已审核的店铺"=>"$admin_path&job=list&type=yz&cityid=$cityid",
			"未审核的店铺"=>"$admin_path&job=list&type=unyz&cityid=$cityid",
			"推荐信息"=>"$admin_path&job=list&type=levels&cityid=$cityid",
			);
...
if($job=="list")
{
	...
	$showpage=getpage("","","$admin_path&job=list&fid=$fid&type=$type&cityid=$cityid&keyword=".urlencode($keyword),$rows,$RS['FOUND_ROWS()']);
