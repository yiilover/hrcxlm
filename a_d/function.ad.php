...
function make_ad_cache(){
	...
	while($rs = $db->fetch_array($query)){
		...
		if(($rs[begintime]&&$rs[begintime]>$timestamp)||($rs[endtime]&&$timestamp>$rs[endtime])){		
			...
			$ad_array = unserialize($rs[adcode]);
			list($_code,$code)=format_ad_code($rs,'',$ad_array);
		...
		if($rs[ifsale]){
			...
			while($_r = $db->fetch_array($query2)){
				$udb = unserialize($_r[u_code]);
				if($rs[type]=='rollpic'){					
					$u_array = $ad_array;
					is_array($ad_array[pic_url]) && is_array($udb[pic_url]) && $u_array[pic_url] = $udb[pic_url] + $ad_array[pic_url];
					is_array($ad_array[pic_alt]) && is_array($udb[pic_alt]) && $u_array[pic_alt] = $udb[pic_alt] + $ad_array[pic_alt];
					is_array($ad_array[pic_link]) && is_array($udb[pic_link]) && $u_array[pic_link] = $udb[pic_link] + $ad_array[pic_link];
				}else{
					$u_array = array_merge($ad_array, $udb );
				}
				list($_code,$code)=format_ad_code($rs,$_r,$u_array);
				if($_r[city_id]){
		...
		if($rs[type]=='word'){
			...
			foreach($pic_url AS $key=>$value){
				$listdb[]=array(
									...
									'title'=>$pic_alt[$key],
									'url'=>"$webdb[www_url]/a_d/a_d_s.php?job=jump&id=$rs[id]&u_id=$_r[u_id]&url=".base64_encode($pic_link[$key]),
									);
			}
			$code="<SCRIPT LANGUAGE='JavaScript' src='$webdb[www_url]/a_d/a_d_s.php?job=js&ad_id=$rs[keywords]'></SCRIPT>";
			eval(str_replace(array('<?php','<?','?>','print','<<<EOT','EOT;'),
			array('EOT;','EOT;','print <<<EOT','$_code.=',"<<<EOT\r\n","\r\nEOT;\r\n"),'print <<<EOT'.En_TruePath($tplpart_code,0).'EOT;'));
