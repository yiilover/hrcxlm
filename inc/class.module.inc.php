	...
	foreach($field_db AS $key=>$rs)
	{
		...
		if($rs[form_type]=='textarea')
		{
			if($type=='show'){	//内容页完整显示
				...
				if(function_exists('highlight_keyword'))$rsdb[$key]=highlight_keyword($rsdb[$key]);
...
function make_template($id){
	require_once($this->f_path."inc/module.class.tpl.php");
	$tplDB = new module_tpl($this);
