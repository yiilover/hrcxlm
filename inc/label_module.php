...
if(!$ch_file&&$chdb[main_tpl]){
	$ch_file=$chdb[main_tpl]; //为兼容其它图片,下载模块
}
...
**/
unset($haveCache);
global $label_hf,$all_label,$head_tpl,$foot_tpl;	//这里不能删除，因为这个文件有些地方是被放在函数里边包含的
$all_label = '';
...
else
{
	...
	$FileName.=(ereg("\.php",basename($WEBURL))?preg_replace("/\.php(.*)/","",basename($WEBURL)):'index')."_".intval($ch_pagetype)."_".intval($ch_module)."_".intval($ch_fid)."_".intval($city_id).'_'.substr(md5(getTpl("index",$ch_file)),0,5).".php";
