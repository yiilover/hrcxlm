...
function post_tpl($rs,&$tplcode){
	...
	elseif($rs[form_type]=='ieeditsimp')
	{
		...
		$is_kindeditor = 1;
		$show.="<textarea name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}' style='visibility:hidden;'>\$rsdb[{$rs[field_name]}]</textarea><SCRIPT LANGUAGE='JavaScript'>XHeditor=$('#atc_{$rs[field_name]}').xheditor({tools:'simple',width:$rs[field_inputwidth],height:$rs[field_inputheight]});</SCRIPT>";
	}
