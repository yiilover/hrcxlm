...
function get_info_url($id,$fid,$cityid='',$zoneid='',$streetid='',$array=array()){
	global $webdb,$city_DB,$BIZ_MODULEDB;
...
function Get_AdInfo($sortid=0,$rows=10,$leng=40,$cityid='city'){
	global $db,$_pre,$timestamp,$city_id,$webdb,$Fid_db;
	
	$cityid=='city' && $cityid=$city_id;
	$cityid>0 && $SQL = " AND A.cityid = '$cityid' ";
	
	if($Fid_db[tableid]){
		$query = $db->query("SELECT A.* FROM {$_pre}buyad A WHERE A.sortid='$sortid' AND A.endtime>$timestamp $SQL ORDER BY A.money DESC,A.aid DESC LIMIT $rows");
		while($rs = $db->fetch_array($query)){
			$iddb[]=$rs[id];
		}
		...
		$query = $db->query("SELECT B.* FROM {$_pre}buyad A LEFT JOIN {$_pre}content B ON A.id=B.id WHERE A.sortid='$sortid' AND A.endtime>$timestamp $SQL ORDER BY A.money DESC,A.aid DESC LIMIT $rows");
		while($rs = $db->fetch_array($query)){
			$listdb[]=$rs;
		}
	}
