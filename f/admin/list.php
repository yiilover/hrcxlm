...
function dowork($id,$job){
	...
	elseif($job=="yz")
	{
		...
		add_user($rsdb[uid],$webdb[PostInfoMoney],"分类信息通过审核增加$webdb[PostInfoMoney]积分");
	}
	elseif($job=="unyz")
	{
		$db->query("UPDATE {$_pre}content$_erp SET yz='0',yzer='$userdb[username]',yztime='$timestamp' WHERE id='$id' ");
		add_user($rsdb[uid],-$webdb[PostInfoMoney],"分类信息取消审核减去$webdb[PostInfoMoney]积分");
	}
	elseif($job=="undel")
	{
	...
	elseif($job=="punish")
	{
		global $Type;
		if($Type==1){
			add_user($rsdb[uid],-abs($webdb[ErrSortMoney]),'处理分类信息扣分');
		}elseif($Type==2){
			add_user($rsdb[uid],-abs($webdb[illInfoMoney]),'处理分类信息扣分');
		}
	}
}
?>