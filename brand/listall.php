...
$query = $db->query("SELECT SQL_CALC_FOUND_ROWS A.*,C.renzheng,C.title AS companyname FROM {$_pre}content A LEFT JOIN {$pre}hy_company C ON A.uid=C.uid $SQL ORDER BY A.$ordertype DESC LIMIT $min,$rows");
