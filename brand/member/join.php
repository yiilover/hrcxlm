...
if($action=="postnew")
{
	...
	refreshto($FROMURL,"<A HREF='?cid=$cid&fid=$fid&id=$id&job=edit'>返回修改</A> <A HREF='../joinshow.php?id=$id' target='_blank'>查看效果</A>",600);
}
...
elseif($action=="edit")
{
	...
	if($postdb[dl_level]!=$rsdb[dl_level]){
		showerr('代理级别不可以修改');
	}
