...
function del_info($id,$rs){
	...
	//删除该品牌下的代理商资料
	$query = $db->query("SELECT * FROM `{$_pre}join` WHERE cid='$id'");
	while($rr = $db->fetch_array($query)){
		$rt = $db->get_one("SELECT * FROM `{$_pre}content_2` WHERE id='$rr[id]'");
		delete_attachment($rt[uid],tempdir($rt[dl_photo]));
		$db->query("DELETE FROM `{$_pre}join` WHERE id='$rr[id]' ");
		$db->query("DELETE FROM `{$_pre}content_2` WHERE id='$rr[id]' ");
	}
}
