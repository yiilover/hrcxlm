...
if($job=='buy'){
	extract($db->get_one("SELECT COUNT(*) AS NUM FROM {$pre}sell_telephone WHERE endtime>$timestamp AND `city_id`='$city_id'"));
	if($NUM>=$webdb[MAX_sell_telephone]){
		showerr("当前城市广告位置已满,你暂时不可以购买!");
	}
	...
	if($step==2){
		...
		if(!$web_admin){
			if($lfjdb[money]<$money){
				...
				add_user($lfjuid,-$money,'购买电话广告位扣分');
			}
		}
	...
	$rsdb = $db->get_one("SELECT * FROM `{$pre}sell_telephone` WHERE id='$id' AND uid='$lfjuid'");

	if($step==2){
		$telephone = filtrate($telephone);
		$title = filtrate($title);
		$about = filtrate($about);
		$db->query("UPDATE `{$pre}sell_telephone` SET `title`='$title',`telephone`='$telephone',`about`='$about' WHERE id='$id' AND uid='$lfjuid' ");
		refreshto("?hack=$hack&",'修改成功',1);
	}

}elseif($action=='del'){

	$db->query("DELETE FROM `{$pre}sell_telephone` WHERE id='$id' AND uid='$lfjuid'");
	refreshto("?hack=$hack&job=list",'删除成功',1);
