...
if($job=="list"&&($Apower[member_list]||$Apower[member_qy_list]))
{
	$field_array = table_field("{$pre}memberdata");
	$field_title = array('username'=>'用户帐号',
					'uid'=>'用户UID',
					'lastip'=>'最后登录IP',
					'regip'=>'注册IP',
					'oicq'=>'QQ号码',
	...
	foreach( $array[field_db] AS $key=>$rs){
		if($rs[form_type]!='textarea'){
			$field_title[$rs[field_name]]=$rs[title];
		}
	}
	...
	if($keywords&&$type){
		...
		}elseif(in_array($type,$field_array)){
			$SQL.=" AND D.`$type`='$keywords' ";
		}
	}
	$select_group=select_group("groupid",$groupid,"index.php?lfj=$lfj&job=list&cityid=$cityid&T=$T");
	...
	$rows=20;
	$min=($page-1)*$rows;
	$showpage=getpage("{$pre}memberdata D","$SQL","index.php?lfj=$lfj&job=$job&type=$type&T=$T&keywords=$keywords&groupid=$groupid&cityid=$cityid",$rows);
	...
	while($rs=$db->fetch_array($query)){
		...
		if(defined('IN_QY')&&$rs[groupid]){
			if($rs[grouptype]==1){
				$rs[yzHy]="<A HREF='index.php?lfj=$lfj&action=yzqy&uid_db[0]=$rs[uid]&T=noyz' title='企业信息已经通过审核,点击即可取消审核'><img src='./images/check_yes-green.gif' border='0'></A>";
			}elseif($rs[grouptype]==-1){
				$rs[yzHy]="<A HREF='index.php?lfj=$lfj&action=yzqy&uid_db[0]=$rs[uid]&T=yz' title='企业信息还没有通过审核,点击即可通过审核'><img src='./images/check_no-blue.gif' border='0'></A>";
			}
			$rs['QY'] = $db->get_one("SELECT * FROM {$pre}hy_company WHERE uid='$rs[uid]'");
			$rs[downqy] = "<A HREF='index.php?lfj=$lfj&action=downqy&uid_db[0]=$rs[uid]' onclick=\"return confirm('你确实要把他降为个人用户吗?点击确定的话,会同时把他的相关企业及商铺信息清空,请慎重考虑!');\" title='把他降为个人用户,同时把他的相关企业及商铺信息清空'><img src='./images/memberdown.gif' border='0'></A>";
			
			$rs[editqy] = "<A HREF='$webdb[www_url]/hy/member/post_company.php?uid=$rs[uid]' title='修改企业信息' target='_blank'><img src='./images/button_edit2.png' border='0'></A>";
...
elseif($job=="editmember"&&($Apower[member_list]||$Apower[member_qy_list]))
{
...
elseif($action=="editmember"&&($Apower[member_list]||$Apower[member_qy_list]))
{
...
elseif($action=="delete"&&($Apower[member_list]||$Apower[member_qy_list]))
{
	if(!$uid_db&&$uid){
		$uid_db[]=$uid;
	}
...
elseif($job=="pwd"&&($Apower[member_list]||$Apower[member_qy_list]))
{
	require(dirname(__FILE__)."/"."head.php");
...
elseif($action=="yz"&&($Apower[member_list]||$Apower[member_qy_list]))
{
	if($T=='yz'){
		$yz=1;
	}else{
...
elseif($action=="unbind"&&($Apower[member_list]||$Apower[member_qy_list]))
{
