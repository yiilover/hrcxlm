...
if(!table_field("{$pre}city",'copyright')){
	$db->query("ALTER TABLE  `{$pre}city` ADD  `copyright` TEXT NOT NULL ;");
	$db->query("ALTER TABLE  `{$pre}city` ADD  `maps` VARCHAR( 50 ) NOT NULL ;");
}
...
function city_cache_guide(){
	...
	while($rs = $db->fetch_array($query)){
		$rs[name]=addslashes($rs[name]);
		unset($_dirname,$_domain,$_tpl,$_T,$_K,$_D,$_HITS,$_LOGOS,$_copyright,$_maps);
		...
		if($rs[logo]){
			$_LOGOS="\r\n\$city_DB[logo][{$rs[fid]}]='$rs[logo]';";
		}
		if($rs[maps]){
			$_maps="\r\n\$city_DB[maps][{$rs[fid]}]='$rs[maps]';";
		}
		if($rs[copyright]){
			$rs[copyright] = addslashes($rs[copyright]);
			$_copyright="\r\n\$city_DB[copyright][{$rs[fid]}]='$rs[copyright]';";
		}
...
$_domain
$_tpl
$_T$_K$_D$_HITS$_LOGOS$_copyright$_maps
";
...
function get_area_guide($cityid=0,$zoneid=0){
	...
	if($zoneid){
		...
		$rs=$db->get_one($SQL);
		$show="<A HREF='?lfj=$lfj&job=zone&fup=$rs[cityid]'>$rs[city]</A> -> <A HREF='?lfj=$lfj&job=street&fup=$rs[zoneid]'>$rs[zone]</A>";
	}elseif($cityid){
		...
		$rs=$db->get_one($SQL);
		$show="<A HREF='?lfj=$lfj&job=city&fup=$rs[proid]'>$rs[pro]</A> -> <A HREF='?lfj=$lfj&job=zone&fup=$rs[cityid]'>$rs[city]</A>";
	}
	return $show;
}
