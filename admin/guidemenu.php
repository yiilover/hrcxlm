...
function menu_cache(){
	global $db,$pre;
	$query = $db->query("SELECT * FROM {$pre}menu WHERE hide=0 AND fid=0 ORDER BY list DESC");
	while($rs = $db->fetch_array($query)){
		$array[]="$rs[name]|$rs[linkurl]|$rs[target]|$rs[color]|$rs[id]|$rs[extend]";
		
		$show='';
		$query2 = $db->query("SELECT * FROM {$pre}menu WHERE hide=0 AND fid='$rs[id]' ORDER BY list DESC");
		while($rs2 = $db->fetch_array($query2)){
			$show.= "$rs2[name]|$rs2[linkurl]|$rs2[target]|$rs2[color]|$rs2[id]|$rs2[extend]\r\n";
		}
		$webdbs["guide_word_$rs[id]"]=$show?addslashes(substr($show,0,-2)):'';
	}
	$webdbs[guide_word]=addslashes( implode("\r\n",$array) );
	write_config_cache($webdbs);	
}
